...
@ApplicationScoped
public class EditMenuBuilder implements MenuFactory.CustomMenuBuilder,
    ...
    public MenuItem build() {
        return new BaseMenuCustom<IsWidget>() {
            ...
            public String getSignatureId() {
                return "org.drools.workbench.screens.guided.dtable.client.editor.menu.EditMenuBuilder";
            }

        };
    }

    void onDecisionTableSelectedEvent( final @Observes DecisionTableSelectedEvent event ) {
        final GuidedDecisionTableView.Presenter dtPresenter = event.getPresenter();
        activeDecisionTable = dtPresenter;
        initialise();
    }

    void onDecisionTableSelectionsChangedEvent( final @Observes DecisionTableSelectionsChangedEvent event ) {
        final GuidedDecisionTableView.Presenter dtPresenter = event.getPresenter();
        activeDecisionTable = dtPresenter;
        initialise();
    }
    ...
    private void enableMenuItems( final List<IGridData.SelectedCell> selections ) {
    ...
    private void setupOtherwiseCellEntry( final List<IGridData.SelectedCell> selections ) {
    ...
    private boolean isOtherwiseEnabled( final List<IGridData.SelectedCell> selections ) {
        if ( selections.size() != 1 ) {
            return false;
        }
    ...
    private boolean isOnlyMandatoryColumnSelected( final List<IGridData.SelectedCell> selections ) {
    ...
    private int findUiColumnIndex( final int modelColumnIndex ) {
        final List<IGridColumn<?>> columns = activeDecisionTable.getView().getModel().getColumns();
